...
function switchMode(mode) {
  ...
  // Full Charge mode: directly set to 90% without picker
  if (mode === 'soc') {
    const targetSoc = 90;
    updateFromPickerValue(targetSoc);
    document.getElementById('picker-scroll').innerHTML = '';
    return;
  }
...
async function startCharging() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      btn.innerHTML = '⏳ WAITING FOR CURRENT...';
      
      // Wait for current to flow (2-3 seconds)
      await waitForChargingCurrent();
...
async function waitForChargingCurrent() {
  const maxAttempts = 6;
  const delayMs = 500;
  ...
  for (let i = 0; i < maxAttempts; i++) {
    ...
    try {
      ...
      if (data.current && data.current !== "0.0 A" && data.current !== "--") {
        console.log('Current detected:', data.current);
        return;
      }
    } catch (err) {
  ...
  console.log('Timeout waiting for current, redirecting anyway');
}
